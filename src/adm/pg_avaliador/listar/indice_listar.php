<?php
		...
		if ($contador_pagina > 1)		{ $lk_anterior="?page=listar&pagina_atual=".$anterior; }
		else							{ $lk_anterior="#"; };
		
		if ($contador_pagina < $total_pagina)		{ $lk_proximo="?page=listar&pagina_atual=".$proximo; }
		else										{ $lk_proximo="#"; };
		...
		{
			$link = "?page=listar&pagina_atual=$lk_total";
