...
if ( isset( $_GET['cp'] ) ) 	{ $kits->find_by_codigo_pessoa( $_GET['cp'], $evento->get_codigo_evento() ); $pessoa->find_by_codigo($_GET['cp']); }
if ( isset( $_GET['em'] ) ) 	{ $kits->find_by_email( $_GET['em'], $evento->get_codigo_evento() ); $pessoa->set_email( $kits->get_email()); $pessoa->set_nome($kits->get_nome()); }

if ( $kits->remove() )
{
		$assunto    = $evento->get_tag_email() . " Cancelamento de Kit";
		$nome = explode(" ", $pessoa->get_nome());
		$nome = $nome[0];
		$mensagem   = "Caro(a) " . $nome . ", \n\nregistramos que a venda de seu kit foi cancelada. Caso haja qualquer problema ou dúvidas, entre em contato imediatamente com a comissão! \n\n" . $evento->get_assinatura_email();

		$pessoa->manda_email($assunto, $mensagem);

	$_SESSION['msg'] = "Kit deletado!";
