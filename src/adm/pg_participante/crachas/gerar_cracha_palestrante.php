...
$zip = new ZipArchive();
$filename = "./crachas/crachas_palestrantes.zip";
...
$consulta = $calendar->find_palestrante_by_evento($codigo_evento);
$count=0;
$extras=0; $nome_minicurso="";
while ($row = mysql_fetch_object($consulta)){

	/*$bc = new Barcode39("PL-".$codigo_evento); 

	$bc->draw('./crachas/codigos_palestrantes/PL-'.$codigo_evento.'.png');
	
	$zip->addFile('./crachas/codigos_palestrantes/PL-'.$codigo_evento.'.png');*/

	
	

	//$nome = explode(' ',$row->autor );
$nome = $row->autor;
	/*$max = sizeof($nome);
	if($max > 2)
	{
	...
	$cracha= "	
	\begin{cracha}{ $camiseta }{ $convocado $diaoral }{ $painel $dia }{  }{ZZ}{ $nome }{ $titulo}{ $nome_minicurso }
	\end{cracha}
	"; 
...
$fp = fopen("crachas/crachas_palestrantes.tex", "w");

$saida=iconv("UTF-8", "ISO-8859-1", $saida);
$escreve = fwrite($fp, $saida);
fclose($fp);

	$zip->addFile('./crachas/crachas_palestrantes.tex');
...
echo "<br /><br />Crachas gerados com sucesso (<a href='crachas/crachas_palestrantes.zip'>crachas_palestrantes.zip</a>)";
?>
